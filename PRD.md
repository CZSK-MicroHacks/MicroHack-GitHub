...
| Goals | Non-Goals |
| --- | --- |
| Unified dashboard for viewing and editing pet profiles plus recent activities. | Full e-commerce checkout, payments, or order management. |
| Reliable CRUD APIs for pets and activities with Cosmos DB persistence. | Native mobile apps; responsive web is sufficient. |
| Capture meaningful telemetry for pet creation and activity logging. | Advanced analytics pipelines or ML-driven recommendations. |
...
| --- | --- | --- |
| Household Pet Owner | Quick overview of pet health, edit details, log walks/feeds. | Can add a new pet and log an activity without errors in <6 minutes. |
...
| Clinic/Trainer Partner | Record structured vet/training visits tied to specific pets. | Logs specialized activities with notes and timestamps. |
...
### Non-Functional
- **Performance**: P95 <300 ms for reads, <500 ms for writes at workshop scale across pets and activities; monitor RU usage.
